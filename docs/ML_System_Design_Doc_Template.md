...
1. **Resumable upload**
   большие видео загружаются multipart-способом, а не одним монолитным запросом.

   Что это закрывает:
   - обрыв сети у агронома
   - нестабильный мобильный интернет в поле
   - падение загрузки на 90-95% пути

   Что это дает:
   - пользователь не начинает загрузку заново
   - снижается число потерянных запусков
   - сервис остается пригодным для работы в реальных полевых условиях

2. **Job idempotency**
   у каждой загрузки или съемки есть свой `job_id`, а результат сохраняется так, чтобы повторный запуск не создавал дубли.

   Что это закрывает:
   - пользователь нажал кнопку запуска дважды
   - задача была перезапущена после сбоя
   - воркер упал и задача ушла в повторную обработку

   Что это дает:
   - одна съемка не превращается в несколько конфликтующих отчетов
   - не происходит двойной записи результатов
   - система безопасно переносит повторные запуски

3. **Retry политики**
   в `Celery` настраиваются автоматические повторные попытки для временных ошибок, например проблем с сетью, S3, Redis или кратковременной перегрузкой воркера.

   Что это закрывает:
   - временная недоступность внешнего сервиса
   - кратковременный сбой одного воркера
   - единичные ошибки чтения файла или обращения к хранилищу

   Что это дает:
   - многие сбои проходят без участия человека
   - уменьшается число "ложных" неуспешных запусков
   - support-команда занимается только реальными, а не случайными инцидентами

4. **Промежуточные артефакты**
   сохраняются видео, извлеченные кадры, отфильтрованные кадры и промежуточные агрегаты.

   Что это закрывает:
   - падение пайплайна на позднем этапе
   - необходимость заново обрабатывать тяжелое видео
   - сложность разбирательства, где именно возникла ошибка

   Что это дает:
   - можно продолжить обработку не с самого начала, а с последнего успешного шага
   - сокращаются вычислительные затраты при перезапуске
   - проще отлаживать ошибки и объяснять результат пользователю

5. **Разделение пайплайна на этапы**
   загрузка, предобработка, инференс и агрегация выполняются как отдельные шаги, а не как один длинный монолитный процесс.

   Что это закрывает:
   - падение одного компонента тянет за собой весь сервис
   - сложно понять, на каком этапе задача сломалась
   - невозможно масштабировать узкое место отдельно

   Что это дает:
   - сбой локализуется в конкретном этапе
   - проще перезапускать только проблемную часть
   - можно отдельно усиливать самые тяжелые шаги, например инференс

6. **Мониторинг и алертинг**
   `Prometheus/Grafana` следят за длиной очереди, временем обработки, числом ошибок и загрузкой воркеров, а `Sentry` фиксирует падения сервисов и исключения в коде.

   Что это закрывает:
   - "тихие" сбои, когда пользователь уже страдает, а команда еще ничего не знает
   - накопление очереди перед сезонным пиком
   - деградацию сервиса без явного падения

   Что это дает:
   - команда видит проблему до массовых жалоб пользователей
   - можно заранее масштабировать воркеры
   - SLA становится управляемым, а не случайным

7. **Понятные статусы для пользователя**
   система должна хранить и показывать статус каждой задачи: `uploaded`, `queued`, `preprocessing`, `inference`, `aggregation`, `done`, `failed`.

   Что это закрывает:
   - пользователь не понимает, зависла система или еще работает
   - support не может быстро ответить, что произошло
   - любой сбой воспринимается как "сервис сломан полностью"

   Что это дает:
   - прозрачность процесса
   - меньше тревоги и повторных кликов "запустить еще раз"
   - проще коммуникация с пользователем при инцидентах

Итоговая логика отказоустойчивости такая:

- `resumable upload` защищает этап передачи данных
- `job idempotency` и `retry` защищают этап выполнения
- промежуточные артефакты и разбиение на этапы защищают этап восстановления после сбоя
- мониторинг и статусы делают сбои наблюдаемыми и управляемыми

То есть асинхронная обработка и очередь задач здесь не абстракция, а конкретная реализация через `FastAPI + Celery + Redis + workers`, дополненная механизмами безопасного повторного запуска и восстановления.
...
“Пилот у нас доказывает, что по фото или видео виноградника модель вообще способна оценивать урожайность. Product-версия отличается не столько новой ML-моделью, сколько доведением решения до состояния сервиса, которым смогут регулярно пользоваться агрономы из разных виноделен.

С точки зрения пользователя в продукте появляется web-интерфейс, куда агроном сам загружает либо видео, либо набор фотографий, указывает минимальные метаданные, например сорт, и запускает анализ. Дальше система сама делает предобработку: проверяет формат файла, режет видео на кадры, отбрасывает плохие или дублирующиеся кадры, запускает детекцию и формирует отчет. Важный момент: продукт должен честно оценивать именно ту зону, которая была реально загружена. Если загружен один куст, результат по кусту; если часть ряда, то по этой части ряда; если есть покрытие всего виноградника, тогда уже можно говорить о прогнозе по всему винограднику.

С точки зрения ML и engineering в product добавляется то, чего в пилоте может не быть: асинхронная обработка, очередь задач, хранение файлов, история запусков, защита от повторного учета одной и той же грозди в соседних кадрах, агрегация результата не только на уровне грозди, но и на уровне сегмента ряда или наблюдаемой зоны. Это важно, потому что бизнесу нужен не список тысяч гроздей, а устойчивый прогноз для планирования рабочих, тары и загрузки переработки.

Отказоустойчивость обеспечивается на нескольких уровнях. Во-первых, загрузка больших файлов должна быть возобновляемой, чтобы обрыв сети в поле не ломал процесс. Во-вторых, задачи обрабатываются асинхронно через очередь, например FastAPI плюс Celery и Redis, поэтому web-интерфейс не блокируется. В-третьих, повторный запуск той же съемки не должен создавать дубли отчетов, то есть обработка должна быть идемпотентной. В-четвертых, сохраняются промежуточные артефакты, чтобы при сбое не пересчитывать весь pipeline заново. И наконец, есть мониторинг и статусы задач, чтобы было понятно, где именно произошла ошибка.

Если совсем коротко, то пилот отвечает на вопрос: работает ли метод. А product отвечает на вопрос: можно ли этим надежно, массово и удобно пользоваться как цифровым сервисом.”